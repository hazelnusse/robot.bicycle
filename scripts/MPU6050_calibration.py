...
def generate_calibration_source(dcm, gyro_offsets):
    s = ("#include \"MPU6050.h\"\n\n" +
         "const float MPU6050::gyro_x_bias = {0}f;\n".format(gyro_offsets[0])  +
         "const float MPU6050::gyro_y_bias = {0}f;\n".format(gyro_offsets[1])  +
         "const float MPU6050::gyro_z_bias = {0}f;\n".format(gyro_offsets[2])  +
         "const float MPU6050::dcm[6] = {" + "{0}f,\n".format(dcm[0, 0]) +
         "                                    {0}f,\n".format(dcm[1, 1]) +
         "                                    {0}f,\n".format(dcm[2, 2]) +
         "                                    {0}f,\n".format(dcm[0, 1]) +
         "                                    {0}f,\n".format(dcm[1, 2]) +
         "                                    {0}f".format(dcm[0, 2]) + "};\n")
    f = open("MPU6050_calibration.cpp", 'w')
    f.write(s)
    f.close()
...
generate_calibration_source(dcm, w_mean.mean(axis=0))
